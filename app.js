...
(function() {
  ...
  display = function(data, name, seed, passelement, qno, answer) {
    var answers, center, div, episodiv, i, input, j, qtext, ref, results;
    episodiv = $('#episopass');
    ...
    if (qtext.match(/\/([^\/]+\.(gif|png|jpg|jpeg))$/i)) {
      ...
      //.css 'margin','4px'
      div = $('<div>').text(qtext).css('background-color', '#ccc').css('width', '400px').css('border-radius', '5px').css('padding', '4px').css('margin', '2 auto');
    }
    center.append(div);
    center.append($('<p>'));
    div = $('<div>');
    ...
    for (i = j = 0, ref = answers.length; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
      input = $('<input>');
      input.attr('type', 'button').attr('value', answers[i]).attr('anumber', i).css('margin', '2pt').css('padding', '1pt').css('width', '50pt').css('height', '25pt').css('border-radius', '4px').css('background-color', '#ccc').click(function(event) {
        var newpass;
        ...
        if (qno < data['qas'].length - 1) {
          ...
          return episodiv.remove(); //  質問ウィンドウを消す
        }
      });
